...
export default defineConfig(({ mode }) => {
  ...
  // If building on GitHub Actions, prefer the repository name as the base
  // so the app works when served from https://<user>.github.io/<repo>/
  const repoName = process.env.GITHUB_REPOSITORY ? process.env.GITHUB_REPOSITORY.split('/')[1] : undefined

  return {
    base: process.env.FIGMA_PUBLIC_URL ? `${process.env.FIGMA_PUBLIC_URL}/` : repoName ? `/${repoName}/` : '/',
    build: {
